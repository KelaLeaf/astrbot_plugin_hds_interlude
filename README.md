...
当前版本 `v1.2.18`，对应上游 `1.0.1-beta6-rebuild`。
...
这套闸门**只对 OneBot / NapCat 家族生效**——telegram、webchat 之类的平台一律放行，开关和名单都跟它们无关。容易搞混的是「开关」和「名单」的关系：

| 配置 | 私聊（OneBot） | 群聊（OneBot） |
| --- | --- | --- |
| 开关**关** | 谁都能聊（名单**完全不看**） | **一个群都不接**，加进群聊白名单也没用 |
| 开关**开** + 名单全空 | 全部拒绝 | 全部拒绝 |
| 开关**开** + 只填机器人账号 | 全部拒绝 | 全部拒绝 |
| 开关**开** + 机器人 + 用户 | 名单里的用户能聊，其他人不接 | 仍然一个群都不接 |
| 开关**开** + 机器人 + 用户 + 群号 | 同上 | 名单里的群会接（**群成员不必在用户白名单里**） |

三条要点：

- **群聊是独立的一条线**：`group_chats` 里没有这个群号，这个群就永远不会被接入——开关开着也一样。
- **白名单不是"分层处理"**：不在名单里的消息 HDSI 一律不管，也不会拿什么"默认提示词"去答——它会被交回管线，由别的插件（或宿主默认 Agent）应答。提示词是全局一套（「提示词」组），不按账号切换。
- **群规则带行为，账号行只是开关**：`group_chats` 每一项可以配触发方式（`mention-only` 只回 @、`auto` 按意愿决定）、防抖、冷却、发言意愿；`bot_accounts` / `user_accounts` 的每一项只有 `enabled` 一个开关，`label` 只是备注（不进模型）。

进了群也不一定有回复：还要过「剧本是 active」「意愿分数够」「不在冷却里」这几道门。为什么没开口，v1.2.17 起日志会直接写明原因。
