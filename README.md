...
当前版本 `v1.3.4`，对应上游 `1.0.1-beta6-rebuild`。
...
- **Agency 行动窗口**。她主动找你之前，先看日程负荷、有没有隐私、设备顺不顺手。忙或者不方便的时候就先不发，过一阵子带着新的生活重新判断。
...
三张名单各自一个「仅处理名单内」开关，**默认都关**。关着的时候名单只用来做**针对性处理**，名单外的照样处理（走默认设定）：

| 开关 | 关闭（默认） | 打开 |
| --- | --- | --- |
| 机器人账号名单 | 名单只作备注 | 只有名单里的登录账号收消息 |
| 用户名单（私聊） | 名单外的人也进叙事，用「故事档案」的默认用户资料与默认关系 | 只有名单里的人能私聊 |
| 群聊名单 | 名单外的群也接，按默认群规则处理（**不 @ 就不说话**） | 只有名单里的群会被接入 |

要点：

- **名单里填了什么就生效什么**：`user_accounts` 那一行决定她怎么称呼你（`label`）以及你的背景与初始关系；`group_chats` 那一行决定那个群的触发方式（`mention-only` / `always`）、防抖、冷却、发言意愿，还有群用途与角色定位。名单外的这些位置就用默认值。
- **三张名单互相独立**：不再要求"机器人和用户都填"，群聊也不看用户名单（群成员不需要私聊授权）。
- **所有平台一视同仁**：telegram、webchat、官方 QQ 的账号与群同样受这三张名单约束。
- 启动了会在日志里打一行现状，例如 `接入与名单：机器人账号名单 1 条→仅名单内；用户名单（私聊） 1 条→名单外也接；群聊名单 2 条→仅名单内`；"开了仅名单内但名单是空的"会用 warn 提醒。那种组合一定不会生效。
- 群聊进来了也不一定有回复：还要过「剧本是 active」「意愿分数够」「不在冷却里」这几道门；为什么没开口，日志会说明原因。

### 模型从哪来

两种都行，也可以混着用：

1. **用 AstrBot 里已经配好的模型**（省事）：模型中心里「主叙事模型」「压缩与总结模型」「Alter 分析模型」，
   「图片理解」里「图片解析模型」，「语音 / 音频理解」里「语音转写模型」，「本地表情包」里「表情包描述模型」，
   以及 Embedding 的「Embedding 模型」，都是下拉选 AstrBot 的模型，**留空就用默认 Provider**。
2. **在模型中心的连接池里自己填**（精细）：每条连接有自己的 endpoint、Key、采样参数、JSON 模式、
   思考开关和用量统计，再用「用于主叙事 / 后台压缩 / Alter / Embedding / 表情包描述 / 侧端识图」勾选用途。
   这种方式支持 failover、round-robin 与费用统计。

指名的那个 AstrBot 模型优先于连接行。选模型时插件会读它的「模型能力」：
声明里没有图片能力的主模型，`hdsi_status` 里会直接告诉你图片会被忽略、建议改用 `sidecar`。

### 结构与节奏

| 分组 | 干什么 |
| --- | --- |
| **共享主剧本** `shared_story` | **一个角色一条时间线**：所有私聊的人在同一个剧本里，各自是一条关系分支（资料、关系、近期状态互相独立）；跨账号联系与管理员。上游刻意硬开启，没有开关。⚠️ `share_participant_details` 默认关闭，此时「记忆整理 / 场景摘要 / 长期事实」看不到对话内容（摘要会退化成空壳）。单人使用建议打开。 |
...
| **网页观察** `browser` | 只读浏览：时机、搜索模板、域名黑白名单、并发与超时、正文长度上限。搜索模板要填**服务端就出结果**的地址：局域网自建 SearXNG（`http://<地址>:<端口>/search?q={query}` 这类，内网主机填进模板即被视为已授权）或 `https://cn.bing.com/search?q={query}`；靠 JS 出结果的（百度/搜狗/Google）抓回来是空白。 |
...
插件详情里的「幕间控制台」页面能看到和改到这些（导出 / 导入配置也在这里，不在聊天里）：

| 面板 | 能看什么 / 能改什么 |
| --- | --- |
| 总览 | 故事与参与者、模型路由、模型能力提示；**十个运行开关点一下即刻生效** |
| 模型 | 每个任务实际用哪个模型、AstrBot 模型与它声明的能力、本次会话 token 用量；**连接池可以增 / 改 / 删** |
| 剧本 | 剧本条目时间线（可翻页）、场景、弧；**顶栏可切换剧本**（标出「主剧本 / 旧剧本」，含已归档的），旧的按账号剧本可以「**设为主剧本**」或「**并入主剧本**」 |
| 记忆 | 长期事实（含置信度与原文引用）、压缩记忆、承诺与意图、设定演化候选、参与者 |
| 情绪 | Alter：当前氛围位移、按来源分的桶、两条曲线（累计位移 / 单轮增量）、触发配置 |
| 日程 | Agency：日程负荷 / 隐私 / 设备三档容量，窗口有效期与推断依据，日程预排摘要 |
| 投递 | 每一次行动到底发出去了没有：按状态筛选，点开看每个分段的结果与失败原因 |
| 数据库 | 13 张表的行数与文件大小 |
| 日志 | 插件最近的运行日志，可筛选、可自动刷新 |
| 配置 | 导出 / 导入配置 |

控制台能改的只有三处：**总览的运行开关**、**模型的连接池**、**「配置」页**（按 schema 全量编辑）；
再加两个剧本整理动作「**设为主剧本**」（选哪一部当主线）与「**并入主剧本**」（把旧剧本并进主线，源剧本转归档）。其余一律只读，
控制台刻意不做成"什么都能改"，那是两个界面各自的职责。

这个页面是随插件一起装的静态资源（构建产物在仓库里），**不需要你自己跑构建**。
...
2. 想让私聊只认名单，就把「接入与名单」里的**仅处理名单内的用户**打开；开着但名单是空的等于谁都不接。
...
5. 是不是开了 `blind_mode.enabled`。开了会屏蔽全部管理命令，日志也收束成心跳，回配置页关掉再重载。
