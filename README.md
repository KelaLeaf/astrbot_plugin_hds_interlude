...
| 命令 | 作用 |
| --- | --- |
| `/hdsi_config_export` | 把当前配置导出成 JSON 文件发给你，同时落到插件数据目录的 `exports/`。 |
| `/hdsi_config_import` | 引用（回复）一份导出的文件即可导入；也可以把 JSON 直接贴在命令后面。导入前会列出会改哪些项，并要你回 y/n。 |

导出的文件带着格式版本号，**以后升级插件仍然能导回来**：老格式的文件、手写的片段、甚至更新版本导出的文件都认；你不认识的键会原样保留，缺的键自动补默认值。换机器、备份配置、给别人抄一份配置都用它。
